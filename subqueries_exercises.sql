...
# | Hilary     | Kambil     |
# +------------+------------+
SELECT * FROM employees WHERE emp_no IN (SELECT emp_no FROM dept_manager WHERE to_date > curDate()) AND gender = 'F';
# BONUS
...
# | Research        |
# +-----------------+
SELECT dept_name FROM departments WHERE dept_no IN (SELECT dept_no FROM dept_manager WHERE emp_no IN (SELECT emp_no FROM employees WHERE gender = 'F') AND to_date > curDate());
